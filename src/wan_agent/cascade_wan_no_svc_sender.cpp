...
int main(int argc, char** argv) {
    ...
    Group<WPCSU, WPCSS> group(
            CallbackSet{}, si, std::vector<DeserializationContext*>{},
            std::vector<view_upcall_t>{[&group](const View& view) {
                node_id_t my_id = getConfUInt32(CONF_DERECHO_LOCAL_ID);
                const std::map<subgroup_id_t, SubgroupSettings>& subgroup_settings_map
                        = view.multicast_group->get_subgroup_settings();
                ...
                while(m_iter != view.my_subgroups.end()) {
                    ...
                    /* use the node with lowest shard_rank or sender_rank to call Replicated<T>.ordered_send to avoid
                     * redundant broadcast. */
                    dbg_default_info("I am in subgroup({}), shard({}), subgroup_settings says my shard_num is {}, "
                                     "shard_rank is {}, sender_rank is {}",
                                     my_subgroup_id, shard_num, subgroup_settings.shard_num,
                                     subgroup_settings.shard_rank, subgroup_settings.sender_rank);

                    /* NOTE: parameter for get_subgroup is "The index of the subgroup within the set of subgroups that
                     * replicate the same type of object", not the global subgroup_id */
                    ...
                    if(my_type_id == 0) {
                        ...
                        dbg_default_info("my_type_id is {}, my_subgroup_index is {}, subgroup_handle says subgroup_id "
                                         "is {}, shard_num is {}",
                                         my_type_id, my_subgroup_index, subgroup_handle.get_subgroup_id(),
                                         subgroup_handle.get_shard_num());
